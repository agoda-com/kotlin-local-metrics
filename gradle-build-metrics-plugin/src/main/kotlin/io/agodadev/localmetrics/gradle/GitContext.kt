...
internal object GitContextReader {
    fun read(rootDirectory: File): GitContext {
        val repository = git(rootDirectory, "config", "--get", "remote.origin.url")
            ?.removeCredentials()
        return GitContext(
            repository = repository,
            repositoryName = repository?.repositoryName(),
            branch = git(rootDirectory, "rev-parse", "--abbrev-ref", "HEAD"),
            commitSha = git(rootDirectory, "rev-parse", "HEAD"),
        )
    }

    private fun git(rootDirectory: File, vararg arguments: String): String? =
        runCatching {
            val process = ProcessBuilder(
                listOf("git", "-C", rootDirectory.absolutePath) + arguments,
            )
